...
@testset "Pkg.add() with julia_version" begin
    ...
    isolate(loaded_depot=true) do
        # Next, test that if we ask for `nothing` it also does, as does something like `Pkg`.
        Pkg.add(["Pkg", "NetworkOptions"]; julia_version=nothing)
        ...
        pkg_block = get_manifest_block("Pkg")
        @test haskey(pkg_block, "uuid")
        @test pkg_block["uuid"] == pkg_uuid
        @test haskey(pkg_block, "version")
    end
end
